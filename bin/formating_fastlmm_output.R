...
# Load the data
output_gwas <- read.table(file = args[2], header = TRUE)
...
MAF <- read.table(file = args[3], header = TRUE)
...
geno_file <- read.table(file = args[4], header = TRUE)
...
markers_file <- readRDS(file = args[5])
...
#### Save output ####
write.table(output_gwas, file = paste0("Formated_", args[1], ".txt"), quote = FALSE, row.names = FALSE)